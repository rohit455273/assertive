test_that("test.character_to_list_of_integer_vectors.strings.returns_list_of_integer_vectors", {
  ...
  expected <- list(1:5, c(1L, NA, 3L, NA, 5L), rep.int(NA_integer_, 5L), NA_integer_, integer(), NA_integer_, 
    c(NA, 2L, NA, 4L, NA))
  names(expected) <- x
  expect_identical(suppressWarnings(character_to_list_of_integer_vectors(x)), expected)
