test_that("test.has_attributes.struct_with_attrs.returns_true_if_attr_exists", 
{
  ...
  expect_equal(has_attributes(x, c("names", "foo", "bar", "baz", NA)), 
    expected)
})
...
test_that("test.has_rows.without_rows.returns_false", {
  x <- 1:10
  actual <- has_rows(x)
  expect_false(actual)
  expect_equal(cause(actual), noquote("The number of rows in x is NULL."))
})

test_that("test.has_rows.zero_rows.returns_false", {
  x <- matrix(numeric(), nrow = 0)
  actual <- has_rows(x)
  expect_false(actual)
  expect_equal(cause(actual), noquote("The number of rows in x is zero."))
})
...
test_that("test.has_terms.without_terms.returns_false", {
  ...
  actual <- has_terms(x)
  expect_false(actual)
  expect_equal(cause(actual), noquote("x has no terms component nor attribute."))
})
