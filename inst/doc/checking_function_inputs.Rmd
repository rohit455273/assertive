...
Consider this function for calculating the geometric mean:
...
deviation.  Type `mad` to see its contents, and run `example(mad)` to get a feel
for how it works.  Update the function (either copy and paste, or use 
`fixInNamespace` if you are feeling fancy) to include some assertions checking 
the inputs.  Hint: Some of the inputs should be numeric; others should be 
logical.  Some should be only allow a single value. [15 mins].
